...
export const useTrackingLogic = (selectedSport: any) => {
  ...
  const [emaSpeed, setEmaSpeed] = useState(0); // Moyenne mobile exponentielle pour vitesse
  const [lastValidSpeed, setLastValidSpeed] = useState(0); // Dernière vitesse valide
  ...
  const gpsPollingInterval = useRef<any>(null); // Polling GPS manuel chaque seconde
  ...
  const getSportConfig = () => {
    ...
    const sportConfigs: Record<string, any> = {
      'Course': {
        ...
        distanceInterval: 0, // 0 = mises à jour temporelles (chaque 1s)
      ...
      'Marche': {
        ...
        distanceInterval: 0, // 0 = mises à jour temporelles uniquement (chaque 1s)
        accuracy: Location.Accuracy.BestForNavigation,
        accuracyThreshold: 20
      },
      'Randonnée': {
  ...
  useEffect(() => {
    if (coords && status === "running") {
      ...
      if (lastCoords) {
        ...
        if (rawSpeedKmh > config.maxSpeed * 1.5) {
          console.log(`🚫 Vitesse rejetée: ${rawSpeedKmh.toFixed(1)} km/h`);
          setLastCoords(coords);
          return;
        }

        let validatedSpeed = rawSpeedKmh;
        setLastValidSpeed(prevValidSpeed => {
          if (prevValidSpeed > 0 && timeDiff < 2) {
            const speedChange = Math.abs(rawSpeedKmh - prevValidSpeed);
            const maxChange = (10 * timeDiff * 3.6);
            if (speedChange > maxChange) {
              console.log(`🚫 Accélération impossible: ${speedChange.toFixed(1)} km/h en ${timeDiff.toFixed(1)}s`);
              validatedSpeed = prevValidSpeed;
              return prevValidSpeed;
            }
          }
          validatedSpeed = rawSpeedKmh;
          return rawSpeedKmh;
        });

        const effectiveSpeed = validatedSpeed;
        const smoothingFactor = Math.min(0.85, Math.max(0.25, timeDiff / (timeDiff + 2)));

        setEmaSpeed(prevEma => {
          const newEma = prevEma === 0
            ? effectiveSpeed
            : (smoothingFactor * effectiveSpeed) + ((1 - smoothingFactor) * prevEma);

          setSpeedHistory(prevHistory => {
            const historySize = 3; // Réduit de 5 à 3 pour moins de latence
            const newHistory = [...prevHistory, effectiveSpeed].slice(-historySize);
            const sorted = [...newHistory].sort((a, b) => a - b);
            const median = sorted[Math.floor(sorted.length / 2)];

            // Réactivité Strava : 70% vitesse actuelle, 30% historique
            const combined = (effectiveSpeed * 0.7) + (newEma * 0.2) + (median * 0.1);
            let finalSpeed = combined < 0.8 ? 0 : combined; // Seuil optimisé marche lente

            const overshootLimit = effectiveSpeed + Math.max(1, effectiveSpeed * 0.35);
            if (finalSpeed > overshootLimit) {
              finalSpeed = overshootLimit;
            }

            setInstantSpeed(finalSpeed);

            setMaxSpeed(prevMax => {
              if (finalSpeed > prevMax && finalSpeed < config.maxSpeed) {
            ...
            console.log(`📊 Vitesse - Brute: ${rawSpeedKmh.toFixed(1)} → EMA: ${newEma.toFixed(1)} → Finale: ${finalSpeed.toFixed(1)} km/h`);
  ...
  const startLocationTracking = async () => {
    try {
      ...
      // Polling GPS manuel chaque seconde pour garantir réactivité type Strava
      // watchPositionAsync ne garantit pas les intervalles sur Android
      const pollGPS = async () => {
        try {
          ...
          console.log('⚠️ Erreur polling GPS:', error);
        }
      };
      ...
      // Puis polling chaque seconde
      gpsPollingInterval.current = setInterval(pollGPS, 1000);

      setWatching(true);
      setIsLocating(false);
      console.log('✅ Polling GPS démarré (chaque 1s)');
      return true;
    } catch (error) {
  ...
  const stopLocationTracking = () => {
    ...
    if (gpsPollingInterval.current) {
      ...
      console.log('⏹️ Polling GPS arrêté');
    }

    // Nettoyer aussi l'ancienne subscription si elle existe
  ...
  const resetTracking = () => {
    ...
    setAvgSpeed(0);
    setSpeedHistory([]);
    setEmaSpeed(0); // Reset de la moyenne mobile exponentielle
    setLastValidSpeed(0); // Reset de la dernière vitesse valide
