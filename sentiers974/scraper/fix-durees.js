...
const Sentier = require('../api/models/Sentier');
require('dotenv').config();

class DureesFixer {
  constructor() {
    console.log('🔧 Correcteur de durées aberrantes');
  }
  ...
  async fixDurees() {
    ...
    for (const sentier of sentiersProblematiques) {
      ...
      console.log(`⚠️  ${sentier.nom}: ${dureeActuelle.toFixed(2)}h`);
      ...
      console.log(`✅ Corrigé: ${sentier.nom} → ${nouvelleduree.heures}h${nouvelleduree.minutes > 0 ? nouvelleduree.minutes : ''}`);
      corrected++;
    }
